...
function sortIngredients(ingredients){
    return /* // TODO*/ [...ingredients].sort(compareIngredientsCB);
}
...
function dishType(dish){
    // TODO
    if(dish.dishTypes == undefined) return ""; // if the dishTypes property is not present 

    if(!dish.dishTypes.find(isKnownTypeCB)) return ""; // if no type is found. 
...
*/
function compareTypeCB(dishA, dishB){
...
function sortDishes(dishes){
    return /*/TODO*/ [...dishes].sort(compareTypeCB);
}
...
function shoppingList(dishes){
    ...
    const arrayOfIngredientArrays= dishes.map(/*TODO pass the callback that transforms a dish to its ingredients */keepJustIngredientsCB);
    const allIngredients= arrayOfIngredientArrays.flat();    
    allIngredients.forEach(/* TODO: pass the callback that treats an ingredient */ingredientCB);
...
function menuPrice(dishesArray){
    ...
    const priceArray = dishesArray.map(dish2PriceCB);
    ...
    return priceArray.reduce(sumPriceCB, 0); 
}
